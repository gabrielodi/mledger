...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if function == "novaManutencao" { //FEITO
		return s.novaManutencao(APIstub, args)
	} else if function == "mudancaProprietario" { //FEITO, MAS FALTA PEGAR A NKEY CORRETA
		return s.mudancaProprietario(APIstub, args)
	} else if function == "mudancaSenha" { //FEITO, MAS FALTA PEGAR A NKEY CORRETA
...
func (s *SmartContract) novaManutencao(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	queryIntString := "{\"selector\":{\"eCnpjCpf\":\"" + args[15] + "\"}}"
	queryString := fmt.Sprintf(queryIntString)
	queryEntidade, _ := getQueryResultForQueryString(APIstub, queryString)
	var responsavel = Entidade{}
	json.Unmarshal(queryEntidade, &responsavel)
	nomeResponsavelRegistro := responsavel.Nome
	if responsavel.Senha != args[16] {
...
func (s *SmartContract) mudancaProprietario(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if (exChassis == false) || (exNovoProprietario == false) || (exIntermediador == false) {
		return shim.Error("Dados incorretos")
	}
...
func (s *SmartContract) mudancaSenha(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	//documento,senha_antiga,senha_nova
	if len(args) != 4 {
	...
	exProprietario := checaExistencia(APIstub, "eCnpjCpf", documento)
	if exProprietario == false {
		return shim.Error("Proprietario Inexistente")
	}

	checkSenha := checaEntidade(APIstub, documento, senhaAntiga, "senha")
	if checkSenha == false {
		return shim.Error("Senha Incorreta")
	}

	//Buscar registro
	queryIntString := "{\"selector\":{\"eCnpjCpf\":\"" + documento + "\"}}"
	queryString := fmt.Sprintf(queryIntString)
	queryEntidade, _ := getQueryResultForQueryString(APIstub, queryString)
	var entidade = Entidade{}
	json.Unmarshal(queryEntidade, &entidade)
	entidade.Senha = senhaNova
...
func checaExistencia(APIstub shim.ChaincodeStubInterface, jsonvar string, parametro string) bool {
...
func checaEntidade(APIstub shim.ChaincodeStubInterface, parametro string, variavel string, tipoVariavel string) bool {
